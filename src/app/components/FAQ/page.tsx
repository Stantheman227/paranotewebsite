...
const FAQItem: React.FC<FAQItemProps> = ({ question, answer }) => {
  ...
  const toggleIconClasses = isOpen ? 'transform rotate-90 duration-300' : 'transform rotate-180 duration-300'
  ...
  return (
    <div className=''>
      <div className={containerClasses}>
        <div className='flex justify-between items-center w-full cursor-pointer' onClick={() => setIsOpen(!isOpen)}>
          ...
          <div className='w-8 h-8 flex bg-gray-100 justify-center items-center rounded-lg relative'>
            ...
            <div className={`h-4 w-[1px] bg-black absolute ${toggleIconClasses}`}></div>
          </div>
        </div>
      </div>
...
const FAQ = () => {
  ...
  const faqs = [
    {
      question: 'Werden die Paragrafen in Echtzeit angezeigt, während ich durch die PDF scrolle?',
      answer:
    ...
    {
      question: 'Kann ich auf alle Gesetzestexte zugreifen, wenn ich offline bin?',
      answer:
    ...
    {
      question: 'Kann ich in der App eigene Anmerkungen zu den Gesetzestexten hinzufügen?',
      answer:
    ...
    {
      question: 'Gibt es eine Suchfunktion für spezifische Paragraphen oder Stichworte?',
      answer:
    ...
    <section className='container mx-auto p-5 space-y-5'>
      <div className='flex flex-col items-center text-center rounded-lg'>
        ...
        <p className='mx-auto mt-4 max-w-2xl text-white text-lg leading-2 '>
          Paranote liefert dir ein Werkzeug, um den Umgang mit Paragrafen und Gesetzestexten so angenehmen wie möglich
          zu gestalten. Optimiere dein Zeitmenagement und schöpfe dein volles Potenzial aus.{' '}
        </p>
      </div>
      <div className='grid grid-cols-2 md:grid-cols-2 gap-4'>
        {faqs.map((faq, index) => (
          <FAQItem
            ref={index === 0 ? squareOneRef : index === 1 ? squareTwoRef : index === 2 ? squareThreeRef : squareFourRef}
            key={index}
